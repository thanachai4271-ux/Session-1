...
with PdfPages('Session1_CustomerAnalysis_short.pdf') as pdf:
    ...
    # 2. & 3. Tables: ใช้ Title และ Headers แบบเต็ม + จัดกึ่งกลาง
    items = [
        ('Gender Distribution (%)', g, ['Gender', 'Percentage (%)']),
        ('Average Spending per Loyalty Tier', t, ['Tier', 'Avg Spending ($)'])
    ]
    
    for title, df, cols in items:
        fig, ax = plt.subplots(figsize=(6,2)); ax.axis('off'); ax.set_title(title, fontsize=13, fontweight='bold', pad=16)
        ax.table(cellText=df.values, colLabels=cols, loc='center', cellLoc='center').scale(1,2)
        pdf.savefig(fig, bbox_inches='tight'); plt.close()

print('✅ Saved Session1_CustomerAnalysis_short.pdf (Full Titles & Headers)')
