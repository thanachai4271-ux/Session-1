...
c = lambda x: pd.to_numeric(x.astype(str).str.replace(r'[^0-9+]','',regex=True),'coerce')
p = p.assign(category=p.category.replace({'Pastry':'Pastries'}), price_c=c(p.price), cost_c=c(p.cost)).assign(profit_margin=lambda x: x.price_c - x.cost_c)
