...
with PdfPages('Session1_ProductPerformancev2.pdf') as d:
      f, a = plt.subplots(); m[m.category.isin(['Pastries','Bread','Tarte'])].groupby('category')['rev'].sum().plot.bar(color=['lightpink','lightblue','lightgreen'], ax=a, title='Total Revenue by Category', ylabel='Total Revenue ($)', rot=0); d.savefig(f, bbox_inches='tight'); plt.close()
