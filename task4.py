...
t3 = m.groupby('product_name')[['quantity','rev']].sum().nlargest(3,'quantity').reset_index().set_axis(['product_name','Total Quantity Sold','Total Revenue'], axis=1)
t3['Total Revenue'] = t3['Total Revenue'].map('${:,.2f}'.format)
