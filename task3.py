...
# ── 2. สร้าง PDF รวมหน้าเดียว ────────────────────────────────────────────────
with PdfPages('Session1_SalesTrendsv2.pdf') as pdf:
    f, ax = plt.subplots(4, 1, figsize=(10, 13), gridspec_kw={'height_ratios': [1, 1, 1, 0.6]})
    ...
    for a, c, cl, t in zip(ax[:3], *zip(*specs)):
        a.plot(m.date, m[c], marker='o', color=cl)
        a.set_title(t, weight='bold')
        a.tick_params('x', rotation=45); a.grid(ls='--')

    # --- วาดตารางด้านล่างสุด ---
    ax[3].axis('off')
    
    # 👉 เติม pad=20 ตรงนี้ครับ เพื่อเว้นช่องไฟระหว่าง Title กับตัวตารางให้สวยงาม
    ax[3].set_title('Top 3 Months', weight='bold', pad=20) 
    
    ax[3].table(cellText=t3.values, colLabels=['Month', 'Total Revenue'], loc='center', cellLoc='center').scale(1, 2)

    pdf.savefig(f, bbox_inches='tight'); plt.close()

print("✅ แก้ไขระยะห่างหัวตารางเรียบร้อย ออกมาสวยงามแน่นอน!")
