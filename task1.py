...
for n, d in v.items():
    ...
    nm = pd.concat([pd.to_numeric(S(x).replace('\$','',regex=True), errors='coerce')<0 for x in nc], axis=1).any(axis=1) if nc else NN    im = (~d['customer_id'].isin(v['customers.csv']['customer_id']) | ~d['product_id'].isin(v['products.csv']['product_id'])) if c=='s' else NN    um = (~d['gender'].isin(['M','F',pd.NA]) | ~S('membership_status').title().isin(['Basic','Silver','Gold','Nan']) | ~S('churned').title().isin(T)) if c=='c' else (~d['category'].isin(['Pastries','Bread','Tarte','Viennoiserie','Pastry',pd.NA]) | ~S('seasonal').title().isin(T) | ~S('active').title().isin(T)) if c=='p' else (~d['payment_method'].isin(['Credit Card','Mobile Pay','Cash',pd.NA]) | ~d['channel'].isin(['Online','In-store',pd.NA]))
    fm = (S('first_name').contains(r'^\s|\s$') | S('email').contains('[A-Z]') | S('phone_number').contains(r'[a-zA-Z\^<|>@]')) if c=='c' else S('cost').contains(r'\$') if c=='p' else N
