...
for n, d in v.items():
    ...
    dm = pd.concat([pd.to_datetime(d[x], errors='coerce').dt.year.between(2010,2025)!=True for x in dc], axis=1).any(axis=1) if dc elselse
    nc = [x for x in d if x in ['quantity','price','cost']]
    nm = pd.concat([pd.to_numeric(S(x).replace(r'\$','',regex=True), errors='coerce')<0 for x in nc], axis=1).any(axis=1) if nc elselse
    im = (~d['customer_id'].isin(v['customers.csv']['customer_id']) | ~d['product_id'].isin(v['products.csv']['product_id'])) if c=='s' elselse
