...
for n, d in v.items():
    ...
    dm = pd.concat([pd.to_datetime(d[x], errors='coerce', format='mixed').dt.year.between(2010,2025)!=True for x in dc], axis=1).any(axis=1) if dc else N    
    ...
    fm = (S('first_name').contains(r'[^a-zA-Z]') | S('email').contains(r'[A-Z]') | ~S('membership_status').title().isin(['Basic','Silver','Gold','nan'])) if c=='c' else (S('product_name').contains(r'[^a-zA-Z]') | S('price').contains(r'^[^$]*$|[\(\)]') | S('cost').contains(r'^[^$]*$|[\(\)]')) if c=='p' else N
